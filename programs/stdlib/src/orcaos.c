...
struct command_argument* orcaos_parse_command(const char* command, int max) {
    ...
    root_command = orcaos_malloc(sizeof(struct command_argument));
    if (!root_command) {
        goto out;
    }
    ...
    token = strtok(NULL, " ");
    while(token != 0) {
        struct command_argument* new_command = orcaos_malloc(sizeof(struct command_argument));
        if (!new_command) {
            break;
        }
...
int orcaos_system_run(const char* command) {
	...
	struct command_argument* root_command_argument = orcaos_parse_command(buf, sizeof(buf));
	if (!root_command_argument) {
		return -1;
	}
	
	return orcaos_system(root_command_argument);
}
