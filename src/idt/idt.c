...
int idt_register_interrupt_callback(int interrupt, INTERRUPT_CALLBACK_FUNCTION interrupt_callback) {
    if (interrupt < 0 || interrupt >=  ORCAOS_TOTAL_INTERRUPTS) {
        return -EINVARG;
    }
...
void isr80h_register_command(int command_id, ISR80H_COMMAND command) {
    if (command_id < 0 || command_id >= ORCAOS_MAX_ISR80H_COMMANDS) {
...
void* isr80h_handle_command(int command, struct interrupt_frame* frame) {
    void* result = 0;
    if (command < 0 || command >= ORCAOS_MAX_ISR80H_COMMANDS) {
        return 0;
    }
...
void* isr80h_handler(int command_id, struct interrupt_frame* frame) {
    ...
    kernel_page();
    task_current_save_state(frame);
    res = isr80h_handle_command(command_id, frame);
    task_page();
    return res;
}
