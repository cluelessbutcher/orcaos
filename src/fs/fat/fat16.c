...
#define ORCAOS_FAT16_FAT_ENTRY_SIZE 0x02
#define ORCAOS_FAT16_BAD_SECTOR 0xFF7
...
static void fat16_init_private(struct disk* disk, struct fat_private* p) {
    memset(p, 0, sizeof(struct fat_private));
    p->cluster_read_stream = diskstreamer_new(disk->id);
    p->fat_read_stream = diskstreamer_new(disk->id);
    p->directory_stream = diskstreamer_new(disk->id);
}
...
int fat16_get_total_items_for_directory(struct disk* disk, uint32_t directory_start_sector) {
    ...
    if (diskstreamer_seek(stream, directory_start_pos) != ORCAOS_ALL_OK) {
        res = -EIO;
        goto out;
    }

    while (1) {
        if (diskstreamer_read(stream, &item, sizeof(item)) != ORCAOS_ALL_OK) {
            res = -EIO;
            goto out;
        }
...
int fat16_get_root_directory(struct disk* disk, struct fat_private* fat_private, struct fat_directory* directory) {
    ...
    struct disk_stream* stream = fat_private->directory_stream;
    if (diskstreamer_seek(stream, fat16_sector_to_absolute(disk, root_dir_sector_pos)) != ORCAOS_ALL_OK) {
        res = -EIO;
        goto err_out;
    }

    if (diskstreamer_read(stream, dir, root_dir_size) != ORCAOS_ALL_OK) {
        res = -EIO;
        goto err_out;
    }
...
int fat16_resolve(struct disk* disk) {
    ...
    if (diskstreamer_read(stream, &fat_private->header, sizeof(fat_private->header)) != ORCAOS_ALL_OK) {
        res = -EIO;
        goto out;
    }
    ...
    if (fat16_get_root_directory(disk, fat_private, &fat_private->root_directory) != ORCAOS_ALL_OK) {
        res = -EIO;
        goto out;
    }
...
static int fat16_cluster_to_sector(struct fat_private* p, int cluster) {
    return p->root_directory.ending_sector_pos + ((cluster - 2) * p->header.primary_header.sectors_per_cluster);
}

static uint32_t fat16_get_first_fat_sector(struct fat_private* p) {
    return p->header.primary_header.reserved_sectors;
}

static int fat16_get_fat_entry(struct disk* disk, int cluster) {
    int res = -1;
    struct fat_private* p = disk->fs_private;
    struct disk_stream* stream = p->fat_read_stream;
    if (!stream) {
        goto out;
    }

    uint32_t fat_table_position = fat16_get_first_fat_sector(p) * disk->sector_size;

    res = diskstreamer_seek(stream, fat_table_position + (cluster * ORCAOS_FAT16_FAT_ENTRY_SIZE));
    if (res < 0) {
        goto out;
    }
...
static int fat16_get_cluster_for_offset(struct disk* disk, int starting_cluster, int offset) {
    int res = 0;
    struct fat_private* p = disk->fs_private;
    int size_of_cluster_bytes = p->header.primary_header.sectors_per_cluster * disk->sector_size;
    ...
    for (int i = 0; i < clusters_ahead; i++) {
        ...
        if (entry == ORCAOS_FAT16_BAD_SECTOR) {
            res = -EIO;
            goto out;
        }
...
static int fat16_read_internal_from_stream(struct disk* disk, struct disk_stream* stream, int cluster, int offset, int total, void* out) {
    ...
    int starting_sector = fat16_cluster_to_sector(p, cluster_to_use);
...
struct fat_item* fat16_find_item_in_directory(struct disk* disk, struct fat_directory* directory, const char* name) {
    ...
    char tmp_filename[ORCAOS_MAX_PATH];
...
int fat16_seek(void* p, uint32_t offset, FILE_SEEK_MODE seek_mode) {
    int res = 0;
    struct fat_file_descriptor* desc = p;
...
int fat16_close(void* p) {
    fat16_free_file_descriptor((struct fat_file_descriptor*) p);
    return 0;
}

int fat16_stat(struct disk* disk, void* p, struct file_stat* stat) {
    int res = 0;
    struct fat_file_descriptor* descriptor = (struct fat_file_descriptor*) p;
